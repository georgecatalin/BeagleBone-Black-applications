/*
===================================================================================================
Name: Program to flash on/off an external LED on BeagleBone Black written in C language
...
int main(int argc, char * argv[])
{
	if(argc!=4)
	{
		...
		printf("The completed usage of this application in command line is: %s <number_of_gpio> <time-in-seconds-on> <time-in-seconds-off>.\n",argv[0]);
		printf("An example of command line instruction to flash external led: %s 60 1 0.5 \n",argv[0]);

	}
	else
	{
        /* *** convert gpio number from string to int *** */
		int converted_to_int=atoi(argv[1]);

		int valid_gpio=verify_if_number_is_valid_gpio(converted_to_int);

		/* *** verify if the number of the gpio is in the list of gpios permitted for BeagleBone Black.
		 * The list with valid numbers of the gpios composed out of BeagleBone Black specifications
		 * ***/
		if(valid_gpio==-1)
		{
		...
		seconds_led_on=convert_string_argument_to_double(argv[2], "on");
		seconds_led_off=convert_string_argument_to_double(argv[3], "off");
